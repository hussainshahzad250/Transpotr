                                                ...
                                                <tbody>
													<thead>
                                                      <tr style="background-color: #cccccc; ">
														...
														<th><div class="center">Time of Cancellation</div></th>
                                                      </tr>
                                                    </thead>
                                                    <%if(request.getAttribute("rejectedBookings") != null){
                                                    	List<CustomerBookingDetails> rejectedBookings = (List<CustomerBookingDetails>)request.getAttribute("rejectedBookings");
                                                    	if(rejectedBookings != null && !rejectedBookings.isEmpty()){
                                                    	for(CustomerBookingDetails bookings : rejectedBookings){%>
                                                    			<tr>
																	...
																	<td><%=bookings.getPickUpTime() %></td>
																	<td><%=DateFormaterUtils.getDateTime(bookings.getUpdatedDate().toString())%></td>                                                                                                 
			                                                    </tr>	
                                                    	<%}}else{%>
                                                    	<tr>
...
<body>
   ...
   <div class="container">  
        <div class="row">
          ...
          <table id="bookingRejectedList"></table>
		  <div id="pbookingRejectedList"></div>         
          </fieldset>        
        </div>
...
jQuery(document).ready(function() {
		...
		$("#bookingRejectedList").jqGrid({
			    url:'/trux/reports/rejectedOrdersGrid',
			    datatype: 'json',
			    mtype: 'GET', 
			    jsonReader : {
					root: "rows",
 				...
 				colNames : ['Customer Name','Customer Phone number','Driver Name','Driver Phone number','Vehicle Type','Vehicle No','To Order Location','From Order Location','Pick-up Time','Time of Cancellation'],
				colModel : [
		                    ...
		                    {name : 'driverName',index : 'driverName',width : 90,hidden : false,editable : true,editrules : {edithidden : false}, editoptions:{readonly: true},cellattr: function (rowId, tv, rawObject, cm, rdata) { return 'style="white-space: normal;"' } }, 
		                    {name : 'driverPhonenumber',index : 'driverPhonenumber',width : 75,hidden : false,editable:true,editrules:{required:true},cellattr: function (rowId, tv, rawObject, cm, rdata) { return 'style="white-space: normal;"' }},   
			                ...
			                {name : 'updatedDateTime',index : 'updatedDateTime',width : 100,hidden : false,editable : true,editrules : {edithidden : false}, editoptions:{readonly: true},cellattr: function (rowId, tv, rawObject, cm, rdata) { return 'style="white-space: normal;"'}},	
		                    ],
				rowNum :10,
				...
				shrinkToFit : true,
				rownumbers : false,
				caption : "Rejected Booking Reports",
				singleselect: true,
				pager : '#pbookingRejectedList',
				viewrecords : true,
   	...
   	$("#bookingRejectedList").jqGrid('navGrid', '#pbookingRejectedList', {
			edit : false,
			add : false,
			del : false,
			exl : true,
			...
			{width : 'auto', serializeDelData: function (postdata) {
				  var selr = jQuery('#bookingRejectedList').jqGrid('getGridParam','selrow'); 
			      var rowData = $("#bookingRejectedList").jqGrid('getRowData');
