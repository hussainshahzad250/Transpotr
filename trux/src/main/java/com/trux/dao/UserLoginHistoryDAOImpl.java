package com.trux.dao;

import java.util.List;

import org.hibernate.Session;
import org.hibernate.SessionFactory;
import org.hibernate.Transaction;
import org.hibernate.criterion.DetachedCriteria;
import org.hibernate.criterion.Projections;
import org.hibernate.criterion.Property; 
import com.trux.model.UserLoginHistory;

public class UserLoginHistoryDAOImpl implements UserLoginHistoryDAO {

private SessionFactory sessionFactory;
	
	public SessionFactory getSessionFactory() {
		return sessionFactory;
	}

	public void setSessionFactory(SessionFactory sessionFactory) {
		this.sessionFactory = sessionFactory;
	}
	 
	 

	public UserLoginHistory saveUserLoginHistory(UserLoginHistory dto) {
		
		
		try{
            Session session=sessionFactory.openSession();
		    Transaction tx=session.beginTransaction();
		    session.save(dto);
		    tx.commit();
		    DetachedCriteria maxId = DetachedCriteria.forClass(UserLoginHistory.class).setProjection(Projections.max("histId"));
		    @SuppressWarnings("unchecked")
			List<UserLoginHistory>  clusterList=session.createCriteria(UserLoginHistory.class).add(Property.forName("histId").eq(maxId)).list();
		    session.close();
		    if(clusterList!=null && clusterList.size()>0){
		    	return	clusterList.get(0);
		    }
		     }catch(Exception er){}
		return null;
	}

	
	public List<UserLoginHistory> getUserLoginHistoryList(Integer userId) {
		
		return null;
	}

	
	public List<UserLoginHistory> getUserLoginHistoryList() {
		
		return null;
	}

}
